    ...
    type SiRenderProps = React.ComponentPropsWithoutRef<'svg'> & {
      /**
    ...
    const defaultColor = '#46E3B7';

    const SiRender: IconType = React.forwardRef<SVGSVGElement, SiRenderProps>(function SiRender({title = 'Render', color = 'currentColor', size = 24, ...others }, ref) {
      if (color === 'default') {
        color = defaultColor;
      }
