    ...
    type SiZulipProps = React.ComponentPropsWithoutRef<'svg'> & {
      /**
    ...
    const defaultColor = '#50ADFF';

    const SiZulip: IconType = React.forwardRef<SVGSVGElement, SiZulipProps>(function SiZulip({title = 'Zulip', color = 'currentColor', size = 24, ...others }, ref) {
      if (color === 'default') {
        color = defaultColor;
      }
